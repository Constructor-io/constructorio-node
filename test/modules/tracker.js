...
function applyParams(parameters, options) {
  ...
  const sendReferrerWithTrackingEvents = options.sendReferrerWithTrackingEvents === false ? false : true; // Defaults to 'true'
...
class Tracker {
  ...
  trackPurchase(parameters) {
    ...
    if (parameters && typeof parameters === 'object' && !Array.isArray(parameters)) {
      ...
      const requestBody = applyParams(bodyParams, { ...this.options,
        requestMethod
      });

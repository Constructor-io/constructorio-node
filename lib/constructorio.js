...
ConstructorIO.prototype = {
   ...
   */
  addOrModifyItem(params, callback) {
    request({
      method: 'PUT',
      url: this.makeUrl('item'),
      auth: this.makeAuthToken(),
  ...
  /**
   * @description Partially updates an item if it exists, or nothing if it does not
   */
   ...
   */
  addOrModifyItemBatch(params, callback) {
    request({
      method: 'PUT',
      url: this.makeUrl('batch_items'),
      auth: this.makeAuthToken(),
  ...
  /**
   * @description Partially updates items if they exist, or nothing if they do not (limit of 1000 items)
   */
  updateItemBatch(params, callback) {
    request({
      method: 'PATCH',
      url: this.makeUrl('batch_items'),
      auth: this.makeAuthToken(),
  ...
  getAutocompleteResults(params, userParams, callback) {
    ...
    const validationResults = validatePersonalizationParams(userParams);

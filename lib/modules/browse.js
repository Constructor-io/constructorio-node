...
const helpers = require('../utils/helpers'); // Create URL from supplied filter name, value and parameters
...
function createBrowseUrl(filterName, filterValue, parameters, options) {
  ...
  if (parameters) {
    const {
      ...
      section,
      fmtOptions
    } = parameters; // Pull page from parameters
...
class Browse {
  ...
  getBrowseResults(filterName, filterValue, parameters) {
    let requestUrl;
    const fetch = this.options && this.options.fetch || fetchPonyfill({
      Promise
    }).fetch;
